...
/*
  ...
  : 메소드 호출과 관련된 개념으로 매개변수를 통해 값을 전달할 때 메모리의 '복사'를 통해 값이 전달
  그래서 다른 메소드(지역)에서 값의 변경이 있더라도 호출한 메소드의 값에는 전혀 영향을 미치지 않는다
  
  메모리적 관점에서 보면 스택영역만 사용하는 케이스로, main과 호출된 메소드는 서로 다른지역이므로 서로에게 영향을 미치지 않는 것이 특징
...
public class E03CallByValue {
	...
	public static void callByValue(int fNum, int sNum) {
		...
		/*
		 Swap: 두 개의 변수가 할당받은 값을 서로 교환하는 것을 말한다.
