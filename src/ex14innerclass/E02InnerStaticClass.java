...
/*
 ...
 ※원래 class앞에는 static을 붙일 수 없으나 내부클래스에는 허용된다
...
class OuterClass{
	...
	void outerMemberMethod() {
		...
		innerStaticClass.innerMemberVar=100; //innerMemberVar 내부정적클래스의 멤버변수(인스턴스형 변수)
		innerStaticClass.innerStaticVar=101; //innerStaticVar 내부정적클래스의 정적멤버변수
	}
	...
	static void outerStaticMethod() {
		...
		/*
		 외부클래스의 정적멤버메소드에서는 내부클래스의 정적멤버만 접근할 수 있다(Method영역에서는 Heap영역을 참조할 수 없다)
		 */
	...
	static class InnerStaticClass{
		...
		static void innerStaticMethod() {
			System.out.println("내부클래스의 정적 메소드 호출");
		}
	}
}
...
public class E02InnerStaticClass {
	...
	public static void main(String[] args) {
		...
		/*
		 내부정적 클래스의 정적멤버에 접근할 때는 외부클래스의 객체 생성 없이
		 클래스명으로 바로 접근가능하다.
		 */
