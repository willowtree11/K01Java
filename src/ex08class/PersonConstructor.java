...
/*
 ...
 생성자(Constructor)
 : 클래스를 객체(인스턴스)화 할 때 자동으로 호출되는 메소드
 자동으로 호출되며, 개발자가 임의로 호출할 수 없다.
 반환값이 없다. 즉, 특정연산 후 값을 반환할 수 없다. void, int 등을 쓰면 안됨.
 그 외에는 메소드가 가진 모든 특성을 가지고 있다.
 (매개변수전달, 오버로딩 등.)
 ...
 용도1 : 동일 클래스 내에서 다른 생성자를 호출할 때 사용한다.
 		...
 		사용법: 
 			this(인자1, 인자2, ...);
 용도2: 생성자 내에서 멤버변수와 매개변수의 이름이 동일할 때 구분하기 위해 사용된다.
 		this.변수=> 멤버변수
 		변수=? 생성자의 매개변수
...
public class PersonConstructor {
	...
	//생성자 메소드(자동으로 생성됨-디폴트생성자): 해당 클래스에 생성자를 정의하지 않은 경우에 
	//아래와 같은 매개변수, 실행부가 없는 생성자가 자동으로 삽입된다.
	...
	public PersonConstructor(String name) {
		this.name=name;//멤버변수=매개변수 -> 멤버변수와 매개변수를 구분하기 위해 'this.'가 사용된다.
		age=1;
		addr="출처불명";
	...
	public PersonConstructor(String _name, int age, String addr) {
		...
		name=_name; //이름이 똑같을 때 앞에 _(언더바)를 써서 구분하기도 한다
		//변수명이 동일한 경우에는 this를 사용하는 것을 권장
		this.age=age;
		this.addr=addr;
	...
	void initialize(String name, int age, String addr) {
		//this(name, age, "미상"); 해당 코드는 생성자 내에서만 사용할 수 있다.
