...
/*
 ...
 : 	같은 타입의 참조변수를 통해 다양한 객체를 이용할 수 있는 특성
 	...
 	- 부모의 참조변수로 자식 객체를 참조
 	1. 이 경우 부모타입의 참조변수는 부모로부터 상속받은 멤버까지만 접근 가능하다.
 	2. 자식에서 오버라이딩 한 메소드가 우선 호출된다.
 	3. 자식에서 새롭게 정의한 멤버에는 접근할 수 없다.
 	
 	- 같은 타입의 클래스로 객체를 참조
 	1. 객체 전체를 접근할 수 있다. 즉 클래스의 일반적인 규칙이 적용된다.
...
class Child extends Parent{
	...
	@Override
	void parentMethod() {
		System.out.println("자식에서 Overriding 한 메소드 " + "parentMethod()");		
	}
	
	//오버로딩 된 메소드(부모랑 상관 없이 확장한 메소드)
	void parentMethod(int childMember) {
		this.childMember=childMember;
		System.out.println("Overloading: 자식에서 확장한 메소드 " + "parentMethod()");
	}
}
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		Parent parent1=homeChild;//자동형변환이 일어남(Up캐스팅) parent타입이 됨
		parent1.parentMember=1;
		//parent1.childMember=1; 접근불가-> 부모타입으로 자식멤버에 접근할 수 없음
		parent1.parentMethod(); //오버라이딩 했으므로 자식 쪽 호출됨
				
		/*
		이질화: 자식에서 새롭게 정의한 멤버(변수, 메소드) 에는 부모타입의 참조변수로 접근 불가
		만약 접근해야 한다면 자식타입으로 형변환(강제 형변환 혹은 Down캐스팅)	이후 자식의 멤버에 접근해야 함.
		 */
		Parent parent2=new Child();
		parent2.parentMember=1;
		parent2.parentMethod();
		
		//자식쪽 멤버에 접근하는 방법1: 형변환 후 바로 접근
		((Child)parent2).childMember=1;
		((Child)parent2).childMethod();
		((Child)parent2).parentMethod();
		
		//자식쪽 멤버에 접근하는 방법2: 자식타입의 인스턴스 변수에 담아서 그 인스턴스 변수로 접근
		 ...
		 */
		//오브젝트 클래스는 자바에서는 모든 클래스가 상속받는 클래스
		Object object=new Child();
		((Parent)object).parentMethod();
		
		Object obj=new String("난 스트링");
	}
