...
/*
 ...
 오버라이딩:	함수명, 매개변수, 반환형까지 모두 동일한 함수를 상속관계에 있는 하위클래스에서 재정의 한 것을 말한다.
 		이 경우 하위클래스에서 정의한 메소드가 상위 클래스의 메소드를 가리게 되므로 항상 최하위 클래스의
 		메소드가 호출되게 된다.
 		※ 오버라이딩은 참조변수의 타입에 영향 받지 않는다
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 참조변수에 상관 없이 항상 최하위에 오버라이딩 된 메소드가 호출된다
		 아래 부분은 모두 c클래스에 생성된 메소드가 호출되게 된다.
		  
		 */
		System.out.println("오버라이딩 처리 된 메소드");
		...
		ref1.loadMethod(); // A클래스의 영역까지만 접근가능함
		...
		//B refId3=refId1; C타입=A타입 -> 불가능(에러발생)
		
		//위에서 에러가 난 부분은 아래처럼 다운캐스팅(강제형변환)하면 참조 가능
		C refId3=(C)refId1; //형변환, 자식타입이 부모타입을 지목하려면 방법이 없는것은 아니다
	}
