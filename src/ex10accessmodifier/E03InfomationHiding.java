...
class FruitSeller{ //과일판매자를 추상화한 클래스
	private int numOfApple = 100; //판매자의 사과 보유 갯수
	private int myMoney = 0; //판매 수익
	private final int APPLE_PRICE; //사과의 단가 //초기화 하지 않은 상수
...
public class E03InfomationHiding {
	...
	public static void main(String[] args) {
		...
		/*
		멤버변수를 private으로 선언하면 외부 클래스에서는 접근이 불가능하므로 아래 코드는 에러가 발생된다.
		즉, 멤버변수의 정보은닉을 통해 비정상적인 접근을 차단하고 멤버메소드를 통해서만 구매가 이뤄질 수 있도록 구현한다.
		 * 
		 * seller1.myMoney += 1000; //판매자1에게 1000원을 지불하고 seller1.numOfApple -= 50; //사과
		 * 50개를 구매했다 buyer.numOfApple +=50;
		 * 
		 * seller2.myMoney += 1000; //판매자2에게 1000원을 지불하고 seller2.numOfApple -= 70; //사과
		 * 70개를 구매했다 buyer.numOfApple +=70; //하지만 구매자의 보유금액은 차감되지 않았다.
		 */		
