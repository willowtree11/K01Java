	...
	public void run() {
		for(int i = 1; i<=1000; i++) {
			...
			/*
			 ...
			 sleep()메소드를 사용하게 되면 실행 중 잠깐씩 Block상태로 전환되어
			 우선순위가 낮은 쓰레드도 가끔 실행될 수 있는 상황이 생기게 된다.
			 */
			try {
				//
				sleep(1); //0.001s 동안 블럭상태로 전환됨
			}
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		 1단계 실행
		 : 우선순위를 부여하지 않은 상태로 쓰레드를 생성하면 동일한 우선순위 5(기본)를 부여받게 된다.
		...
		*/
		...
		/*
		 2단계 실행
		 : 우선순위 부여를 위해 정수를 사용한다. 우선순위는 1~10 사이에서 부여할 수 있고
		 높을수록 우선순위도 높아지게 된다.
		 ※ 단, 우선순위가 높다고 무조건 먼저 실행되지는 않는다.
		 전체적인 확률이 높아지게되어 먼저 실행된 가능성이 높아진다고 생각하면 된다.	 
