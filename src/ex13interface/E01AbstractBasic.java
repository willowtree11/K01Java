...
abstract class AbsClass1{
	...
	//클래스에 하나 이상의 앱스트랙 메소드를 포함하게 되면 무조건 abstract 선언 필요
...
class AbsClassChild1 extends AbsClass1{
	...
	/*
	 추상메소드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 강제사항x
...
/*
 부모가 가진 추상메소드를 그대로 상속받게 되므로 해당 클래스도 반드시 abstract로 선언해야 한다.
 단 이 경우 객체를 생성하지 못하게 되므로 부모의 abstract 메소드를 오버라이딩 하면 된다.  
 */
class AbsClassChild2 extends AbsClass2{
	@Override
	void absMethod(int number) {
		System.out.println("부모클래스에서 오버라이딩 한 메소드로 없을 땐 에러발생");
	}
...
public class E01AbstractBasic {
	...
	public static void main(String[] args) {
		...
		//추상클래스를 상속한 하위클래슨느 객체생성이 가능하다.
		AbsClassChild1 absClassChild1 = new AbsClassChild1();
		
		/*
		 추상클래스로 인스턴스 생성은 할 수 없으나 참조변수로 사용하는 것은 가능하다		 
		 */
		...
		/*
		 추상클래스 내에 선언된 정적멤버는 오버라이딩의 대상이 아니므로
