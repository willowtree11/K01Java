...
/*
  ...
  캡슐화(Encapsulation)
  :	여러가지 비즈니스 로직을 하나로 묶는다는 의미와 업무의 순서까지 고려한 형태의 로직을 구성한다는 의미를 가지고 있다.
...
/*
 ...
 업무의 순서를 고려하여 관련된 메소드들을 하나의 클래스로 정의하였다.
 차후에는 별도의 분석없이 해당 클래스의 메소드만 호출하면 업무는 오류없이 진행될 것  
 */
class EncapsulLogic{
...
public class E05Encapsulation {
	/*
	  ...
	  업무의 진행순서가 중요하다고 했을 때 캡슐화 이전의 코드에서는 개발자가 순서를 지키지 않는 상태에서도 실행이 가능한 코드가 된다.
