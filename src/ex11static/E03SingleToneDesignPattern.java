...
/*
  ...
  싱글톤 디자인 패턴
  : 클래스를 설계하는 디자인 패턴의 하나로써 하나의 인스턴스 즉 하나의 메모리를 생성해 이를 공유해서
  사용하고자 할 때 사용하는 패턴이다.
...
class SingleTone{
	int shareVar;
	/*
	 JVM(자바 가상머신)에 의해 프로그램이 시작될 때 정적변수는 미리 메소드영역에 생성되어 사용할 준비를 마치게 된다.
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		//일반적인 방식의 객체생성이므로 객체를 생성할 때 마다 새로운 참조값(주소값)을 할당받는 인스턴스가 된다.
		...
		nst1.instVar = 200;
		System.out.println("nst2= "+nst1);
		
		//SingleTone st1 = new SingleTone(); 오류발생 : 생성자 호출이 불가능 private이기 때문
		/*
		 
		 생성자를 접근할 수 없는 대신 getInstance() 메소드를 통해서만 객체의 참조값을 반환받을 수 있다.
		 반환 받을 때는 항상 같은 객체를 반환받게 된다.
		...
		System.out.println(String.format("st2의 주소: %s", st3));
