...
/*
TreeSet<T> 컬렉션
- 트리라는 자료구조를 기반으로 한 Set컬렉션.
- Set의 기본적인 특성은 동일하나 Tree알고리즘을 통해 정렬되어 저장. 정렬의 기준은 개발자가 직접 정의할 수 있음
- 정렬의 기준을 정하기 위해 Comparable<T> 인터페이스를 구현해야 한다.
...
class MyString implements Comparable<MyString>{
	...
	/*
	 정렬의 기준을 위해 오버라이딩 한 메소드로 가나다와 같은 사전순, 문자열의 길이 등 다양하게 설정 가능.
	...
	public int compareTo(MyString paramStr) {
		...
		//문자열의 알파벳 순서대로 정렬한다
		/*
		 * if(str.compareTo(paramStr.str)>0) return 1; else
		 * if(str.compareTo(paramStr.str)<0) return -1; else return 0;
		 */
	}
	@Override
...
public class Ex04TreeSetMain {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 - 문자열을 compareTo()를 비교해서 정렬할 때는 모든 문자열이 Set에 삽입된다. 
		 */
		TreeSet<MyString> tree2=new TreeSet<MyString>();
		tree2.add(new MyString("Orange")); //문자열 길이 6
